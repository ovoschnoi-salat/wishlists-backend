...
func (s *Service) DenyUserIncomingFriendsRequest(c *gin.Context) {
	...
	if err != nil {
		errors.SendResponse(c, http.StatusBadRequest, codes.InvalidRequestParametersErrCode, fmt.Errorf("invalid friend_id: %w", err))
		return
	}
	...
	if err != nil {
		errors.SendResponse(c, http.StatusInternalServerError, codes.InternalErrCode, fmt.Errorf("cannot deny friend request: %w", err))
		return
	}
	if count == 0 {
		errors.SendResponse(c, http.StatusInternalServerError, codes.InternalErrCode, errors.New("cannot deny friend request: no rows updated"))
		return
	}
